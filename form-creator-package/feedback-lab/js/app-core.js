...
(function () {
  ...
  async function generateStudent(student, mcqKey, openDefs, onProgress) {
    ...
    const mcqNorm = wrongMcq.map(q => { const f = mcq.find(m => String(m.q) === String(q)) || {}; const chosen = (student.mcq[q] || "").trim(); return { q: String(q), chosen, unanswered: !chosen, correct: (mcqKey[q] || "").trim(), error: f.error || "", hint: f.hint || (chosen ? "Take another look at this one and check the force involved." : "This question was left unanswered — read it again and complete the correction below.") }; });
...
function createFeedbackDoc() {
  ...
  window.FeedbackApp = { generateStudent, buildAppsScript, buildWordDoc, serKey, parseKey, serOpen, parseOpen, serStudents, parseStudents, sameChoice, normChoice };
})();
