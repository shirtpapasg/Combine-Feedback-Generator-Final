...
FEEDBACK LOGIC
MCQ (only the wrong ones are listed for you): each wrong MCQ comes with its QUESTION text — read it and write feedback about THAT question's science, never a guessed topic. Write a short, encouraging "error" (what idea was likely confused) and a "hint" that nudges them to the right thinking — WITHOUT naming the correct option.
Open-ended:
...
- INCORRECT -> status "incorrect": give "error" = a PRECISE diagnosis that names WHICH PART of the answer went wrong — never a vague "check your working" or "you may have forgotten something". Diagnose against these three checks, and state the one(s) that failed:
  ...
  MANDATORY SEPARATION: the "error" field must END with the diagnosis — never with a question. Any question you want the pupil to think about belongs in "ponder" (or "hint"), NEVER inside "error". Every "explain"-type question with status "incorrect", "partial" or "blank" MUST have a non-empty "ponder"; every non-explain one MUST have a non-empty "hint". Do not merge the two fields.
...
Include EVERY open-ended question id given, in order. Include only the wrong MCQs listed.`;
  ...
  const mcqList = wrongMcq.length
    ? wrongMcq.map(q => {
        // Without the question's own wording Claude invents a topic (a Plant Systems
        // paper came back with feedback about magnets), so send the stem too.
        const stem = (window.__mcqStems || {})[String(q)] || "";
        return `- Q${q}: pupil chose ${student.mcq[q] || "(blank)"}` +
          (stem ? `\n  QUESTION: ${String(stem).slice(0, 420)}` : "");
      }).join("\n")
    : "(none wrong)";
