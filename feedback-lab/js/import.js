...
(function () {
  ...
  function inferType(stem) {
    return /\b(explain|why|account for|suggest|reason|how (?:does|do|can|is|are|will|would|might)|state .*reason|with reference|what (?:causes|caused)|give a reason)\b/i.test(stem || "") ? "explain" : "short";
  }
  ...
  function scanResponses(input, roster, kindOverrides) {
    ...
    // These identify the ADMIN columns. Every pattern must match the WHOLE header —
    // a question stem containing "water level" or "Name the process" would otherwise
    // be mistaken for the Class/Name column and dropped from the paper entirely.
    headers.forEach((h, i) => {
      const t = String(lower[i] || "").trim().replace(/\s+/g, " ");
      if (/^(timestamp|date|points|total points|score|total|username|marks)\.?$/.test(t)) ignore.add(i);
      if (/^e-?mail( address)?\.?$/.test(t) || t === "email") { emailCol = i; ignore.add(i); }
      if (surnameCol < 0 && /^(surname|family name|last name)\.?$/.test(t)) surnameCol = i;
      // "Reg" on its own is the common Singapore heading — accept it, plus S/N and No.
      if (regCol < 0 && /^(reg|reg no|reg number|register|register no|register number|index|index no|index number|admin no|s\/?n|no)\.?$/.test(t)) regCol = i;
      if (nameCol < 0 && /^((full |your |pupil'?s? |student'?s? |short )?name)\.?$/.test(t)) nameCol = i;
      if (classCol < 0 && /^(class|level|class level|form class)\.?$/.test(t)) classCol = i;
    });
    ...
    // A form can legitimately split one printed part across two questions ("5(a) identify P",
    // "5(a) identify Q"). Both headers parse to the same id, and without this the second
    // column silently overwrote the first, so one pupil answer vanished from the feedback.
    const seenSaqIds = {};
    cols.forEach(c => {
      if (c.kind !== "saq") return;
      if (seenSaqIds[c.id] == null) { seenSaqIds[c.id] = [c]; return; }
      seenSaqIds[c.id].push(c);
      c.baseId = c.id;                       // answer-key lookups still use the printed id
      c.id = c.id + "-" + seenSaqIds[c.baseId].length;
    });
    // Two questions sharing a printed number usually differ only in their last few
    // words ("…identify part P" / "…identify part Q"), so a truncated stem makes them
    // look like duplicates. Show the part that actually differs.
    Object.keys(seenSaqIds).forEach(id => {
      const g = seenSaqIds[id];
      if (g.length < 2) return;
      const stems = g.map(c => String(c.stem || ""));
      let p = 0;
      while (p < stems[0].length && stems.every(s => s[p] === stems[0][p])) p++;
      const cut = Math.max(0, stems[0].lastIndexOf(" ", Math.max(0, p - 1)));
      if (cut > 12) g.forEach((c, k) => { c.stemShort = "… " + stems[k].slice(cut).trim(); });
    });
    ...
    return { tsv: out.join("\n"), mcqIds: mcqCols.map(c => c.id), saqDefs, note, missing, unmatchedRows, unmatchedCols, cols: cols.map(c => ({ id: c.id, kind: c.kind, header: c.header, stem: c.stemShort || c.stem })) };
  }
  ...
  async function structureMcq(keyText) {
    const txt = await ask(
      ...
      "CRITICAL — MCQ keys are usually printed as a GRID of several question/answer column PAIRS side by side, so one text line holds more than one question. A line like \"1 2 6 4\" means question 1 = 2 AND question 6 = 4; \"3 1 8 1\" means question 3 = 1 AND question 8 = 1. Read each line as consecutive (question, answer) pairs — never as one question followed by a run of answers, and never renumber. Check your result: the question numbers must run 1,2,3,… with no gaps and no number larger than the number of MCQ in the paper. " +
  ...
  async function structureMarking(keyText, paperText, saqDefs, onStatus) {
    ...
    saqDefs.forEach(d => {
      // "5(a)-2" is a form that split one printed part in two — look up its printed id
      const base = String(d.id).replace(/-\d+$/, "");
      if (out[d.id]) d.marking = out[d.id];
      else if (out[base]) d.marking = out[base];
    });
    return saqDefs;
  }
